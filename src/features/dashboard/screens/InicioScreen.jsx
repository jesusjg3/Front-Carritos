...
export default function InicioScreen() {
      ...
      import("../../../core/services/echo").then(({ createEcho }) => {
        ...
        channel.listen(".driver.disconnect.approved", () => {
          ...
          showAlert(
            ...
            "El administrador aprobó tu desconexión.",
            "success",
          );
        });
        ...
        channel.listen(".driver.disconnect.rejected", () => {
          ...
          import("react-native").then(({ Alert }) => {
            Alert.alert(
              "Desconexión Rechazada",
              "El administrador denegó tu solicitud de desconexión.",
            );
          });
  ...
  useEffect(() => {
    ...
    const isSelectingDestination =
      isPasajero && destinoSeleccionado && !activeTrip;
    ...
    if (isIdle) {
      ...
      if (isPhase1) {
        ...
        if (isConductor) {
          start = {
            lat: getVal(ubicacion?.latitude),
            lng: getVal(ubicacion?.longitude),
          };
        } else {
          ...
          if (
            ...
            const knownDriver = nearbyDrivers.find(
              (d) => d.id == activeTrip.driver?.id,
            );
            if (knownDriver) {
        ...
        padding = 180;
          ...
          lat: getVal(
            activeTrip.destination_lat || activeTrip.destination?.lat,
          ),
          lng: getVal(
            activeTrip.destination_lng || activeTrip.destination?.lng,
          ),
        };
      ...
      if (showUserMarker && ubicacion) {
        if (isConductor && isOnline && !user?.vehicle_maintenance) {
          const escapedIconUrl = CARRITO_MARKER_BASE64.replace(
            /\\/g,
            "\\\\",
          ).replace(/'/g, "\\'");
      ...
      if (validCoords) {
        ...
        const shouldAnimateZoom =
          lastRouteRef.current?.phase !== (isPhase2 ? "phase2" : "phase1");

        // Determine if we need to draw multi route (carpooling)
        if (
          isConductor &&
          ...
          const routeChanged =
            lastRouteRef.current?.startLat !== start.lat ||
            lastRouteRef.current?.startLng !== start.lng ||
            lastRouteRef.current?.endLat !== end.lat ||
            lastRouteRef.current?.endLng !== end.lng ||
            lastRouteRef.current?.phase !== currentPhase;
  ...
  const confirmRequestTrip = async (passengersCount) => {
    if (!destinoSeleccionado || !ubicacion) {
      showAlert(
        "Ubicación Requerida",
        "Necesitamos tu ubicación y un destino.",
        "warning",
      );
      return;
    }
    ...
    if (distFromCenter > radiusKm) {
      Alert.alert(
        "Fuera de zona",
        `Estás fuera de la zona de servicio permitida (${radiusKm} km).`,
      );
      return;
    }
  ...
  const handleRequestDisconnect = async (reason) => {
    ...
    try {
      const response = await fetch(
        `${API_ROUTES.BASE_URL}/driver/request-disconnect`,
        {
          method: "POST",
          headers: {
      ...
      if (!response.ok) {
        ...
        showAlert(
          "Error",
          data.error || "No se pudo solicitar la desconexión.",
          "error",
        );
      }
    } catch (e) {
  ...
  if (isWaitingDisconnect) {
    return (
      ...
      >
        ...
        >
          Tu solicitud ha sido enviada al administrador. Por favor, no cierres
          la app.
        </Text>
      </SafeAreaView>
    );
  ...
  if (activeTrip) {
    return (
      ...
      >
        ...
        {/* Floating Incoming Request for Carpooling */}
        <IncomingRequestCard
